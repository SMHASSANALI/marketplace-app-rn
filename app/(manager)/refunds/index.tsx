/**
 * Manager — Refunds & Exchanges list.
 *
 * Only accessible when the Owner has granted the 'refunds_exchanges' permission.
 * Managers can create new requests and approve/reject them — same workflow as
 * the Owner's refunds screen.
 */
...
function RERow({ item }: { item: RefundExchangeFull }) {
  ...
  return (
    <Pressable
      onPress={() => router.push(`/(manager)/refunds/${item.id}`)}
      style={({ pressed }) => ({
...
// ---------------------------------------------------------------------------
// Create RE flow (same logic as owner's screen)
// ---------------------------------------------------------------------------
...
function CreateREFlow({ onClose }: { onClose: () => void }) {
  ...
  function toggleItem(liId: number, maxQty: number) {
    setSelected(prev => {
      if (prev[liId]) { const n = { ...prev }; delete n[liId]; return n; }
  ...
  async function handleSubmit() {
    ...
    const items = Object.entries(selected).map(([liId, { qty }]) => {
      ...
      return { order_line_item_id: li.id, product_id: li.product_id, return_quantity: qty };
    });
        ...
        <>
          ...
          {orderInfo.line_items.map(li => {
            ...
            return (
              ...
              >
                <Ionicons name={isSelected ? 'checkbox' : 'square-outline'} size={20} color={isSelected ? COLORS.brand : COLORS.muted} />
                <Text style={{ flex: 1, fontSize: FONT_SIZES.sm, color: COLORS.text }}>
                  {li.product_emoji} {li.product_name}
                </Text>
                <Text style={{ fontSize: FONT_SIZES.xs, color: COLORS.muted }}>Qty: {li.quantity}</Text>
              </Pressable>
            );
          })}
        </>
...
export default function ManagerRefundsScreen() {
      ...
      <>
        ...
        <Screen padded>
          <EmptyState
            ...
            description="The owner has not granted you access to Refunds & Exchanges."
          />
        </Screen>
      </>
    );
            ...
            ListEmptyComponent={
              !showForm ? (
                <EmptyState
                  ...
                  description="Create one using the button above. Only delivered orders are eligible."
                />
              ) : null
            }
